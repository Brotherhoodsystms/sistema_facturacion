...
{
  public static function obtenerDetalleTemp($id_usuario)
  {
    try {
      $sql = "SELECT * FROM tbl_detalletemporal WHERE temp_idusuario=$id_usuario ORDER BY temp_id DESC";
  ...
  {
    try {
      $sql = "INSERT INTO tbl_detalletemporal(temp_serialproducto,temp_descripcion,temp_cantvender,
      temp_precio,temp_descuento,temp_total,temp_idproducto,temp_idusuario,temp_bodegaid_o,temp_descripcionu,temp_tipo_producto)
      VALUES(:temp_serialproducto,:temp_descripcion,:temp_cantvender,:temp_precio,:temp_descuento,
      :temp_total,:temp_idproducto,:temp_idusuario,:temp_bodegaid_o,:temp_descripcionu,:temp_tipo_producto)";
      $query = Conexion::obtenerConexion()->prepare($sql);
      ...
      $query->bindParam(":temp_bodegaid_o", $data["bodega_id"]);
      $query->bindParam(":temp_descripcionu", $data["ubicacion_descripcion"]);
      $query->bindParam(":temp_tipo_producto", $data["temp_tipo_producto"]);
  ...
  {
    try {
      $sql = "DELETE FROM tbl_detalletemporal
  ...
  public static function actualizarDetalletempId($id,$nuevoestock,$totalVenta,$precio,$descuento)
  {
    try {
      $sql = "UPDATE tbl_detalletemporal SET temp_cantvender=:temp_cantvender, temp_total=:temp_total,temp_precio=:temp_precio,temp_descuento=:temp_descuento
      WHERE temp_id=:temp_id";
      $query = Conexion::obtenerConexion()->prepare($sql);
      $query->bindParam(":temp_id", $id);
      $query->bindParam(":temp_cantvender", $nuevoestock);
      $query->bindParam(":temp_total", $totalVenta);
      $query->bindParam(":temp_precio", $precio);
      $query->bindParam(":temp_descuento", $descuento);
      $query->execute();
      return true;
    } catch (\Throwable $ex) {
      return $ex->getMessage();
  ...
  public static function sumarDetalleTemp($id_usuario)
  {
    try {
      $sql = "SELECT SUM(temp_total) as total FROM tbl_detalletemporal where temp_idusuario=:temp_idusuario";
      $query = Conexion::obtenerConexion()->prepare($sql);
  ...
  public static function sumarDetalleFactura($factura_id)
  {
    ...
    public static function buscarImpuestoProducto($idProducto)
    {
      try {
        $sql = "SELECT producto_porcentaje FROM tbl_producto WHERE producto_id=:producto_id";
        $query = Conexion::obtenerConexion()->prepare($sql);
        $query->bindParam(":producto_id", $idProducto);
        $query->execute();
        return  $query->fetch(PDO::FETCH_ASSOC);
  ...
  public static function obtenerProductosCombo($id)
  {
    try {
      $sql = "SELECT * FROM tbl_combos
      WHERE combo_id=:producto_id";
      $query = Conexion::obtenerConexion()->prepare($sql);
  ...
  public static function obtenerTipoProducto($id)
  {
    try {
      $sql = "SELECT producto_tipo FROM tbl_producto
      WHERE producto_id=:producto_id";
      $query = Conexion::obtenerConexion()->prepare($sql);
      $query->bindParam(":producto_id", $id);
      $query->execute();
  ...
  {
    try {
      $sql = "SELECT producto_stock,producto_descripcion,producto_tipo FROM tbl_producto
      WHERE producto_id=:producto_id";
      $query = Conexion::obtenerConexion()->prepare($sql);
      $query->bindParam(":producto_id", $id);
      $query->execute();
  ...
  public static function obtenerProductosVenta()
  {
    try {
      $sql = "SELECT producto_id,producto_codigoserial,producto_descripcion,ROUND(producto_precio_menor,2) AS producto_precio_menor,producto_precio_mayor,ROUND(producto_stock,2) AS producto_stock,producto_tipo,producto_codigoreferencia,producto_imagen FROM tbl_producto WHERE producto_status='1'";
      $query = Conexion::obtenerConexion()->prepare($sql);
      $query->execute();
      return  $query->fetchall(PDO::FETCH_ASSOC);
    } catch (\Throwable $ex) {
  ...
  public static function obtenerProductoCodigo($id)
  {
    try {
      $sql = "SELECT * FROM tbl_producto
      WHERE producto_codigoserial=:producto_codigoserial";
  ...
  public static function guardarFactura($data)
  {
    try {
      ...
      $sql = "INSERT INTO tbl_factura(factura_serie,factura_fechagenerada,factura_subtotal,factura_iva,factura_total,formpago_id,cliente_id,comprobante_id,emisor_id,factura_usuario_id)
       VALUES(:factura_serie,:factura_fechagenerada,:factura_subtotal,:factura_iva,:factura_total,:formpago_id,:cliente_id,:comprobante_id,:emisor_id,:factura_usuario_id)";
      ...
      $query->bindParam(":factura_iva", $data["factura_iva"]);
