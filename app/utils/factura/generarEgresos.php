...
if (empty($_REQUEST['cl']) || empty($_REQUEST['f'])) {
  ...
  $sql2 = "SELECT * FROM `tbl_tempubicacion` as tu INNER JOIN tbl_producto as p on p.producto_id=tu.temp_ubica_productoid INNER JOIN tbl_categoria as c on c.categoria_id=p.categoria_id INNER JOIN tbl_proveedor as pro on pro.proveedor_id=p.proveedor_id INNER JOIN tbl_lote as l on l.lote_id=p.lote_id INNER JOIN tbl_bodega as b on b.bodega_id=tu.tem_ubica_bodegaid INNER JOIN tbl_sucursal as s on s.sucursal_id=b.sucursal_id WHERE tu.tem_idtransaccion=:reserva_id";
...
*/
    ob_start();
    include(dirname('__FILE__') . '/Egresos.php');
    $html = ob_get_clean();
    ...
    $dompdf->stream('Egresos' . $noIngreso . '.pdf', array('Attachment' => 0));
