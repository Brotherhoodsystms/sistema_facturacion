...
if (empty($_REQUEST['cl']) || empty($_REQUEST['f'])) {
  ...
  if ($datos > 0) {
    ...
    $sql3 = "SELECT establecimiento.codigo as establecimientocodigo, puntoemision.* FROM tbl_establecimiento as establecimiento INNER JOIN tbl_ptoemision as puntoemision on establecimiento.id=puntoemision.establecimiento_id where establecimiento.emisor_id=:emisor_id";
...
*/
    ob_start();
    include(dirname('__FILE__') . '/reserva.php');
    $html = ob_get_clean();
    ...
    file_put_contents($rutaGuardado . 'factura_' . $noFactura . '.pdf', $output);
