...
if (empty($_REQUEST['cl']) || empty($_REQUEST['f'])) {
  echo "No es posible generar el reporte escoja el filtro que desea buscar";
} else {
  ...
  if ($noFactura == 1) {
    ...
    $sql2 = "SELECT * FROM tbl_factura f JOIN tbl_cliente c
    ON f.cliente_id = c.cliente_id JOIN tbl_formapago g
    ON f.formpago_id= g.formpago_id WHERE c.cliente_ruc=:reserva_id AND f.emisor_id=:emisor_id";
    $query2 = Conexion::obtenerConexion()->prepare($sql2);
    $query2->bindParam(":reserva_id", $parametro);
    $query2->bindParam(":emisor_id",$emisor_id);
    $detalleTodos =
      $query2->execute();
  } else if ($noFactura == 3) {
    ...
    $sql2 = "SELECT * FROM tbl_factura f JOIN tbl_cliente c ON f.cliente_id = c.cliente_id
    JOIN tbl_formapago g ON f.formpago_id= g.formpago_id
      WHERE f.factura_serie=:reserva_id";
    $query2 = Conexion::obtenerConexion()->prepare($sql2);
    $query2->bindParam(":reserva_id", $parametro);
    $detalleTodos =
      $query2->execute();
    ...
    AND f.factura_fechagenerada <= :reserva_fechafinal";
    $query2 = Conexion::obtenerConexion()->prepare($sql2);
  ...
  if ($detalleTodos > 0) {
    ...
    ob_start();
    include(dirname('__FILE__') . '/ReporteFactura.php');
    $html = ob_get_clean();
    ...
    $dompdf->stream('Reporte_Factura_' . $noFactura . '.pdf', array('Attachment' => 0));
